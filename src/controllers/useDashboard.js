...
export function useDashboard(t, streakData) {
  ...
  const recent = useMemo(() => [...transactions].sort((a, b) => String(b.dateISO).localeCompare(String(a.dateISO)) || Number(b.id) - Number(a.id)).slice(0, 4).map((tx) => {
    const cat = categories.find((item) => item.id === tx.categoryId);
    const dayLabel = tx.daysAgo === 0 ? d.today : tx.daysAgo === 1 ? d.yesterday : `${tx.daysAgo} ${d.daysAgoLabel || "ngày trước"}`;
    const when = tx.createdTime ? `${dayLabel} · ${tx.createdTime}` : dayLabel;
